...
func (c *Controller) waitUntilPaused(db *api.MariaDB) error {
	klog.Infof("waiting for pods for MariaDB %v/%v to be deleted\n", db.Namespace, db.Name)
	...
	klog.Infof("waiting for services for MariaDB %v/%v to be deleted\n", db.Namespace, db.Name)
...
func (c *Controller) waitUntilRBACStuffDeleted(db *api.MariaDB) error {
	klog.Infof("waiting for RBACs for MariaDB %v/%v to be deleted\n", db.Namespace, db.Name)
...
func (c *Controller) waitUntilStatefulSetsDeleted(db *api.MariaDB) error {
	klog.Infof("waiting for statefulsets for MariaDB %v/%v to be deleted\n", db.Namespace, db.Name)
...
func (c *Controller) waitUntilDeploymentsDeleted(db *api.MariaDB) error {
	klog.Infof("waiting for deployments for MariaDB %v/%v to be deleted\n", db.Namespace, db.Name)
	return wait.PollImmediate(kutil.RetryInterval, kutil.GCTimeout, func() (bool, error) {
...
func (c *Controller) haltDatabase(db *api.MariaDB) error {
	...
	// delete appbinding
	klog.Infof("deleting AppBindings of MariaDB %v/%v.", db.Namespace, db.Name)
	...
	// delete PDB
	klog.Infof("deleting PodDisruptionBudget of MariaDB %v/%v.", db.Namespace, db.Name)
	if err := c.Client.
		PolicyV1beta1().
	...
	klog.Infof("deleting StatefulSets of MariaDB %v/%v.", db.Namespace, db.Name)
	if err := c.Client.
		AppsV1().
	...
	klog.Infof("deleting Deployments of MariaDB %v/%v.", db.Namespace, db.Name)
	if err := c.Client.
		AppsV1().
	...
	klog.Infof("deleting RoleBindings of MariaDB %v/%v.", db.Namespace, db.Name)
	if err := c.Client.
		RbacV1().
	...
	klog.Infof("deleting Roles of MariaDB %v/%v.", db.Namespace, db.Name)
	if err := c.Client.
		RbacV1().
		Roles(db.Namespace).
		DeleteCollection(
	...
	klog.Infof("deleting ServiceAccounts of MariaDB %v/%v.", db.Namespace, db.Name)
	if err := c.Client.
		CoreV1().
	...
	klog.Infof("deleting Services of MariaDB %v/%v.", db.Namespace, db.Name)
	svcs, err := c.Client.
		CoreV1().
	...
	klog.Infof("deleting Monitoring resources of MariaDB %v/%v.", db.Namespace, db.Name)

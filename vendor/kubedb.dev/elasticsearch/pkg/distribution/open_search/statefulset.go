...
func (es *Elasticsearch) getVolumes(esNode *api.ElasticsearchNode) ([]core.Volume, *core.PersistentVolumeClaim, error) {
	...
	if !es.db.Spec.DisableSecurity {
		...
		// To load configuration changes to the security plugin (securityadmin.sh),
...
func (es *Elasticsearch) getContainers(esNode *api.ElasticsearchNode, envList []core.EnvVar) ([]core.Container, error) {
	...
	volumeMount := []core.VolumeMount{
		{
			Name:      "data",
			MountPath: api.ElasticsearchOpenSearchDataDir,
		},
		{
			Name:      "esconfig",
			MountPath: api.ElasticsearchOpenSearchConfigDir,
		},
	}
	...
	if es.db.Spec.ConfigSecret != nil {
		volumeMount = core_util.UpsertVolumeMount(volumeMount, core.VolumeMount{
			Name:      "custom-config",
			MountPath: path.Join(api.ElasticsearchOpenSearchConfigDir, api.DBCustomConfigName),
		})
	}
	...
	if !es.db.Spec.DisableSecurity {
		volumeMount = core_util.UpsertVolumeMount(volumeMount, []core.VolumeMount{
			{
				Name:      "security-config",
				MountPath: api.ElasticsearchOpenSearchSecurityConfigDir,
			},
			{
				Name:      es.db.CertSecretVolumeName(api.ElasticsearchTransportCert),
				MountPath: es.db.CertSecretVolumeMountPath(api.ElasticsearchOpenSearchConfigDir, api.ElasticsearchTransportCert),
			},
			// To load configuration changes to the security plugin (securityadmin.sh),
			// user must provide the admin certificate to the tool.
			// So we must mount the admin certificates, so that the cert files usable from inside the pods.
			{
				Name:      es.db.CertSecretVolumeName(api.ElasticsearchAdminCert),
				MountPath: es.db.CertSecretVolumeMountPath(api.ElasticsearchOpenSearchConfigDir, api.ElasticsearchAdminCert),
			},
		}...)
		...
		if es.db.Spec.EnableSSL {
			volumeMount = core_util.UpsertVolumeMount(volumeMount, core.VolumeMount{
				Name:      es.db.CertSecretVolumeName(api.ElasticsearchHTTPCert),
				MountPath: es.db.CertSecretVolumeMountPath(api.ElasticsearchOpenSearchConfigDir, api.ElasticsearchHTTPCert),
			})
		}
	}

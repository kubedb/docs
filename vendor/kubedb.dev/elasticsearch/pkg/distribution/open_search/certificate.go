...
func (es *Elasticsearch) EnsureCertSecrets() error {
	...
	// script path: /plugins/opensearch_security/tools/securityadmin.sh
	//	ref: https://opensearch.github.io/for-elasticsearch-docs/docs/security/configuration/security-admin/
...
func (es *Elasticsearch) createAdminCertSecret(caKey *rsa.PrivateKey, caCert *x509.Certificate, cPath string) error {
	cSecret, err := es.findSecret(es.db.GetCertSecretName(api.ElasticsearchAdminCert))
	if err != nil {
		return err
	}
	...
	if cSecret == nil {
		// create certs here
		if err := pkcs8.CreateClientCertificate(string(api.ElasticsearchAdminCert), cPath, es.db, caKey, caCert); err != nil {
			return err
		}
		...
		secret := &core.Secret{
			ObjectMeta: metav1.ObjectMeta{
				Name:   es.db.GetCertSecretName(api.ElasticsearchAdminCert),
				Labels: es.db.OffshootLabels(),
			},
			Type: core.SecretTypeTLS,
			Data: data,

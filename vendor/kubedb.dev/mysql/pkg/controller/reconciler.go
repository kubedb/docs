...
func (c *Reconciler) ReconcileNodes(db *api.MySQL) (*api.MySQL, kutil.VerbType, error) {
	...
	if db.Spec.TLS != nil && db.Spec.TLS.IssuerRef != nil {
		...
		if !ok {
			...
			// drop the MySQL object from work queue (i.e. return nil with no error).
			// When any secret owned by this MySQL object is created/updated,
			// this MariaDB object will be enqueued again for processing.
			klog.Infoln(fmt.Sprintf("Required secrets for MySQL: %s/%s are not ready yet", db.Namespace, db.Name))

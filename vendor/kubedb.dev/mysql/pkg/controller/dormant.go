...
func (c *Controller) waitUntilHalted(db *api.MySQL) error {
	klog.Infof("waiting for pods for MySQL %v/%v to be deleted\n", db.Namespace, db.Name)
	...
	klog.Infof("waiting for services for MySQL %v/%v to be deleted\n", db.Namespace, db.Name)
...
func (c *Controller) waitUntilStatefulSetsDeleted(db *api.MySQL) error {
	klog.Infof("waiting for statefulsets for MySQL %v/%v to be deleted\n", db.Namespace, db.Name)
...
func (c *Controller) secretsUsedByPeers(meta metav1.ObjectMeta) (sets.String, error) {
	...
	for _, my := range dbList {
		if my.Name != meta.Name {
			secretUsed.Insert(my.Spec.GetPersistentSecrets()...)
		}
	}
...
func (c *Controller) haltDatabase(db *api.MySQL) error {
	...
	// delete appbinding
	klog.Infof("deleting AppBindings of MySQL %v/%v.", db.Namespace, db.Name)
	...
	// delete PDB
	klog.Infof("deleting PodDisruptionBudget of MySQL %v/%v.", db.Namespace, db.Name)
	if err := c.Client.
		PolicyV1beta1().
	...
	klog.Infof("deleting StatefulSets of MySQL %v/%v.", db.Namespace, db.Name)
	if err := c.Client.
		AppsV1().
	...
	klog.Infof("deleting RoleBindings of MySQL %v/%v.", db.Namespace, db.Name)
	if err := c.Client.
		RbacV1().
	...
	klog.Infof("deleting Roles of MySQL %v/%v.", db.Namespace, db.Name)
	if err := c.Client.
		RbacV1().
		Roles(db.Namespace).
		DeleteCollection(
	...
	klog.Infof("deleting ServiceAccounts of MySQL %v/%v.", db.Namespace, db.Name)
	if err := c.Client.
		CoreV1().
	...
	klog.Infof("deleting Services of MySQL %v/%v.", db.Namespace, db.Name)
	svcs, err := c.Client.
		CoreV1().
	...
	klog.Infof("deleting Monitoring resources of MySQL %v/%v.", db.Namespace, db.Name)

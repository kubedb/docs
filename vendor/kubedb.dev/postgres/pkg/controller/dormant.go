...
func (c *Controller) waitUntilPaused(db *api.Postgres) error {
	klog.Infof("waiting for pods for Postgres %v/%v to be deleted\n", db.Namespace, db.Name)
	...
	klog.Infof("waiting for services for Postgres %v/%v to be deleted\n", db.Namespace, db.Name)
...
func (c *Controller) waitUntilStatefulSetsDeleted(db *api.Postgres) error {
	klog.Infof("waiting for statefulsets for Postgres %v/%v to be deleted\n", db.Namespace, db.Name)
...
func (c *Controller) haltDatabase(db *api.Postgres) error {
	...
	// delete appbinding
	klog.Infof("deleting AppBindings of Postgres %v/%v.", db.Namespace, db.Name)
	...
	// delete PDB
	klog.Infof("deleting PodDisruptionBudget of Postgres %v/%v.", db.Namespace, db.Name)
	if err := c.Client.
		PolicyV1beta1().
	...
	klog.Infof("deleting StatefulSets of Postgres %v/%v.", db.Namespace, db.Name)
	if err := c.Client.
		AppsV1().
	...
	klog.Infof("deleting RoleBindings of Postgres %v/%v.", db.Namespace, db.Name)
	if err := c.Client.
		RbacV1().
	...
	klog.Infof("deleting Roles of Postgres %v/%v.", db.Namespace, db.Name)
	if err := c.Client.
		RbacV1().
		Roles(db.Namespace).
		DeleteCollection(
	...
	klog.Infof("deleting ServiceAccounts of Postgres %v/%v.", db.Namespace, db.Name)
	if err := c.Client.
		CoreV1().
	...
	klog.Infof("deleting Services of Postgres %v/%v.", db.Namespace, db.Name)
	svcs, err := c.Client.
		CoreV1().
	...
	klog.Infof("deleting Monitoring resources of Postgres %v/%v.", db.Namespace, db.Name)

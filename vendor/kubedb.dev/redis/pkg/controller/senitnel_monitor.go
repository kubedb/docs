...
func (c *Controller) newSentinelMonitorController(db *api.RedisSentinel) (mona.Agent, error) {
...
func (c *Controller) addOrUpdateSentinelMonitor(db *api.RedisSentinel) (kutil.VerbType, error) {
	agent, err := c.newSentinelMonitorController(db)
	if err != nil {
...
func (c *Controller) deleteSentinelMonitor(db *api.RedisSentinel) error {
	agent, err := c.newSentinelMonitorController(db)
	if err != nil {
		return err
	}
...
func (c *Controller) getOldAgentForSentinel(db *api.RedisSentinel) mona.Agent {
...
func (c *Controller) setNewAgentForSentinel(db *api.RedisSentinel) error {
	service, err := c.Client.CoreV1().Services(db.Namespace).Get(context.TODO(), db.StatsService().ServiceName(), metav1.GetOptions{})
	if err != nil {
		return err
	}
...
func (c *Controller) manageSentinelMonitor(db *api.RedisSentinel) error {
	oldAgent := c.getOldAgentForSentinel(db)
	if db.Spec.Monitor != nil {
		if oldAgent != nil && oldAgent.GetType() != db.Spec.Monitor.Agent {
			if _, err := oldAgent.Delete(db.StatsService()); err != nil {
				klog.Error("error in deleting Prometheus agent:", err)
			}
		}
		if _, err := c.addOrUpdateSentinelMonitor(db); err != nil {
			return err
		}
		return c.setNewAgentForSentinel(db)

...
def go_build(name, goos, goarch, main, compress=False, upx=False):
    ...
    if goos == 'alpine':
        ...
        cmd = "docker run --rm -u {uid} -v /tmp:/.cache -v {repo_root}:/go{repo_dir} -w /go{repo_dir} -e {cgo_env} golang:1.10.0-alpine {goc} build -o {bindir}/{name}-{goos}-{goarch}{ext} {cgo} {ldflags} {main}".format(
            repo_root=REPO_ROOT,
            repo_dir=repo_dir,
            uid=uid,
            name=name,

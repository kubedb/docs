...
`spec.replicas` specifies the total number of primary and standby nodes in Postgres database cluster configuration. One pod is selected as Primary and others are acted as standby replicas. KubeDB uses Pod Disruption Budget to ensure more than half of the replicas are still available during [involuntary disruptions](https://kubernetes.io/docs/concepts/workloads/pods/disruptions/#voluntary-and-involuntary-disruptions).
...
If you want to use an existing or custom secret, please specify that when creating the Postgres object using `spec.databaseSecret.secretName`. This Secret should contain superuser *username* as `POSTGRES_USER` key and superuser *password* as `POSTGRES_PASSWORD` key. Secrets created by users are not managed by kubeDB, and therefore, wont be modified or deleted by the kubeDB operator (version >= 0.13.0).
...
If this field is left empty, kubeDB will generate a service account name, and create a service account by that name. Role and RoleBinding that provide necessary access permissions will also be generated automatically for that service account.

If a service account name is given, but there's no existing service account by that name, kubeDB will create one, and Role and RoleBinding that provide necessary access permissions will also be generated for that service account.

If a service account name is given, and there's an existing service account by that name, kubeDB will use that existing service account. If the service account is managed by kubeDB, additional Role and RoleBinding that provide necessary access permissions will also be generated by kubeDB. Otherwise, if the existing Service Acoount is managed by the users, they will have to provide these necessary access permissions manually.

Guide on how to provide these necessary access permissions manually to gain finer control over role based access using custom Service Account can be found [here](/docs/guides/postgres/custom-rbac/using-custom-rbac.md). 

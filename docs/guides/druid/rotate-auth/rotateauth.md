...
2. **User Defined:** The user can create their own credentials by defining a Secret of type `kubernetes.io/basic-auth` containing the desired `username` and `password`, and then reference this Secret in the `MariaDBOpsRequest`.
...
```shell
 $ kubectl apply -f kubectl apply -f https://github.com/kubedb/docs/raw/{{< param "info.version" >}}/docs/guides/mariadb/quickstart/overview/examples/Mariadb-rotate-auth-generated.yaml
...
> You cannot change the database `username`, but you can update the `password` while keeping the existing `username`.
```shell
...
```shell
$ kubectl apply -f https://github.com/kubedb/docs/raw/{{< param "info.version" >}}/docs/guides/mariadb/quickstart/overview/examples/Mariadb-rotate-auth-generated.yaml
...
Kind:         MariaDBOpsRequest
Metadata:
  Creation Timestamp:  2025-07-11T12:11:21Z
  Generation:          1
  Resource Version:    659881
  UID:                 8e80c141-d54c-4b14-9049-4b3f4fc3be5a
Spec:
  Apply:  IfReady
  Authentication:
    Secret Ref:
...
Status:
  Conditions:
    Last Transition Time:  2025-07-11T12:11:51Z
    Message:               Controller has started to Progress the MariaDBOpsRequest: demo/mdops-rotate-auth-user
    Observed Generation:   1
    Reason:                Running
    Status:                True
    Type:                  Running
    Last Transition Time:  2025-07-11T12:11:54Z
    Message:               Successfully referenced the user provided authSecret
    Observed Generation:   1
    Reason:                patchedSecret
    Status:                True
    Type:                  UpdateCredential
    Last Transition Time:  2025-07-11T12:12:01Z
    Message:               evict pod; ConditionStatus:True; PodName:sample-mariadb-0
    Observed Generation:   1
    Status:                True
    Type:                  EvictPod--sample-mariadb-0
...
Events:
  Type     Reason                                                     Age   From                         Message
  ----     ------                                                     ----  ----                         -------
  Normal   Pending                                                    22m   KubeDB Ops-manager Operator  Skipping MariaDBOpsRequest demo/mdops-rotate-auth-user, concurrent MariaDBOpsRequest is not allowed for same database, will retry after 30 seconds.
  Normal   Pending                                                    22m   KubeDB Ops-manager Operator  Skipping MariaDBOpsRequest demo/mdops-rotate-auth-user, concurrent MariaDBOpsRequest is not allowed for same database, will retry after 30 seconds.
  Normal   Starting                                                   22m   KubeDB Ops-manager Operator  Start processing for MariaDBOpsRequest: demo/mdops-rotate-auth-user
  Normal   Starting                                                   22m   KubeDB Ops-manager Operator  Pausing MariaDB databse: demo/sample-mariadb
  Normal   Successful                                                 22m   KubeDB Ops-manager Operator  Successfully paused MariaDB database: demo/sample-mariadb for MariaDBOpsRequest: mdops-rotate-auth-user
  Normal   Starting                                                   22m   KubeDB Ops-manager Operator  Restarting Pod: demo/sample-mariadb-0
  Warning  evict pod; ConditionStatus:True; PodName:sample-mariadb-0  22m   KubeDB Ops-manager Operator  evict pod; ConditionStatus:True; PodName:sample-mariadb-0
  Warning  get pod; ConditionStatus:True; PodName:sample-mariadb-0    22m   KubeDB Ops-manager Operator  get pod; ConditionStatus:True; PodName:sample-mariadb-0
  Warning  get pod; ConditionStatus:True; PodName:sample-mariadb-0    21m   KubeDB Ops-manager Operator  get pod; ConditionStatus:True; PodName:sample-mariadb-0
  Normal   Successful                                                 21m   KubeDB Ops-manager Operator  Successfully restarted MariaDB pods for MariaDBOpsRequest: demo/mdops-rotate-auth-user
  Normal   Successful                                                 21m   KubeDB Ops-manager Operator  Successfully rotate MariaDB auth for MariaDBOpsRequest: demo/mdops-rotate-auth-user
  Normal   Starting                                                   21m   KubeDB Ops-manager Operator  Resuming MariaDB database: demo/sample-mariadb
  Normal   Successful                                                 21m   KubeDB Ops-manager Operator  Successfully resumed MariaDB database: demo/sample-mariadb
  Normal   Successful                                                 21m   KubeDB Ops-manager Operator  Controller has successfully rotate MariaDB auth secret
...
Mariadbopsrequest.ops.kubedb.com "mdops-rotate-auth-generated" "mdops-rotate-auth-user" deleted

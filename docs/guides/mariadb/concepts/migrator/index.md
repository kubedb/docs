...
page describes the `Migrator` object for a **MariaDB** source and target.
...
`connectionInfo` (also under `spec.target.mariadb`) tells the Migrator how to connect to the MariaDB
...
- `maxConnections` — limits the number of concurrent connections the Migrator opens to this MariaDB
  instance.

---
title: Upgrading MariaDB group replication minor version
menu:
  docs_{{ .version }}:
    identifier: my-upgrading-mariadb-minor-group
...
# Upgrade minor version of MariaDB Group Replication

This guide will show you how to use `KubeDB` enterprise operator to upgrade the minor version of `MariaDB` Group Replication.
...
metadata:
  ...
  selfLink: /apis/catalog.kubedb.com/v1alpha1/mariadbversions/5.7.29
...
The above `spec.upgradeConstraints.denylist` of `5.7.29` is showing that upgrading below version of `5.7.29` is not possible for both group replication and standalone. That means, it is possible to upgrade any version above `5.7.29`. Here, we are going to create a `MariaDB` Group Replication using MariaDB  `5.7.29`. Then we are going to upgrade this version to `5.7.31`.
...
```bash
$ kubectl create -f https://github.com/kubedb/docs/raw/{{< param "info.version" >}}/docs/examples/mariadb/upgrading/minorversion/group_replication.yaml
...
Here, we are going to upgrade the `MariaDB` group replication from `5.7.29` to `5.7.31`.
...
```bash
$ kubectl apply -f https://github.com/kubedb/docs/raw/{{< param "info.version" >}}/docs/examples/mariadb/upgrading/minorversion/upgrade_minor_version_group.yaml
mariadbopsrequest.ops.kubedb.com/my-upgrade-minor-group created
```
...
You can see from the above output that the `MariaDBOpsRequest` has succeeded. If we describe the `MariaDBOpsRequest`, we shall see that the `MariaDB` group replication is updated with the new version and the `StatefulSet` is created with a new image.
...
Metadata:
  ...
  Self Link:         /apis/ops.kubedb.com/v1alpha1/namespaces/demo/mariadbopsrequests/my-upgrade-minor-group
...
Status:
  Conditions:
    ...
    Message:               Controller has started to Progress the MariaDBOpsRequest: demo/my-upgrade-minor-group
    ...
    Type:                  HaltDatabase
    Last Transition Time:  2020-08-12T08:34:33Z
    Message:               MariaDB version upgrading stated for MariaDBOpsRequest: demo/my-upgrade-minor-group
    Observed Generation:   1
    Reason:                DatabaseVersionUpgradingStarted
    Status:                True
    Type:                  Upgrading
    Last Transition Time:  2020-08-12T08:42:23Z
    Message:               Image successfully updated in MariaDB: demo/my-group for MariaDBOpsRequest: my-upgrade-minor-group 
    Observed Generation:   1
    ...
    Type:                  ResumeDatabase
    Last Transition Time:  2020-08-12T08:42:23Z
    Message:               Controller has successfully scaled/upgraded the MariaDB demo/my-upgrade-minor-group
    Observed Generation:   2
...
Events:
  ...
  ----    ------      ----   ----                        -------
  Normal  Starting    7m59s  KubeDB Enterprise Operator  Start processing for MariaDBOpsRequest: demo/my-upgrade-minor-group
  Normal  Starting    7m59s  KubeDB Enterprise Operator  Pausing MariaDB databse: demo/my-group
  Normal  Successful  7m59s  KubeDB Enterprise Operator  Successfully halted MariaDB database: demo/my-group for MariaDBOpsRequest: my-upgrade-minor-group
  Normal  Starting    7m59s  KubeDB Enterprise Operator  Upgrading MariaDB images: demo/my-group for MariaDBOpsRequest: my-upgrade-minor-group
  ...
  Normal  Successful  9s     KubeDB Enterprise Operator  Image successfully updated in MariaDB: demo/my-group for MariaDBOpsRequest: my-upgrade-minor-group
  Normal  Successful  9s     KubeDB Enterprise Operator  Image successfully upgraded for Pod: demo/my-group-0
  Normal  Successful  9s     KubeDB Enterprise Operator  Image successfully upgraded for Pod: demo/my-group-2
  Normal  Successful  9s     KubeDB Enterprise Operator  Image successfully upgraded for standalone/master: demo/my-group-1
  Normal  Starting    9s     KubeDB Enterprise Operator  Resuming MariaDB database: demo/my-group
  Normal  Successful  9s     KubeDB Enterprise Operator  Successfully resumed MariaDB database: demo/my-group
  Normal  Successful  9s     KubeDB Enterprise Operator  Controller has Successfully upgraded the version of MariaDB : demo/my-group
```
...
$ kkubectl exec -it -n demo my-group-0 -c mariadb -- mariadb -u root --password=OUta0G6QnyBuKmi0 --host=my-group-0.my-group-gvr.demo -e "select * from performance_schema.replication_group_members"
mariadb: [Warning] Using a password on the command line interface can be insecure.
+---------------------------+--------------------------------------+------------------------------+-------------+--------------+
| CHANNEL_NAME              | MEMBER_ID                            | MEMBER_HOST                  | MEMBER_PORT | MEMBER_STATE |
+---------------------------+--------------------------------------+------------------------------+-------------+--------------+

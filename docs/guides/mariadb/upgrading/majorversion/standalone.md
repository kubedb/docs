---
title: Upgrading MariaDB standalone major version
menu:
  docs_{{ .version }}:
    identifier: my-upgrading-mariadb-major-standalone
    name: Standalone
...
# Upgrade major version of MariaDB Standalone

This guide will show you how to use `KubeDB` enterprise operator to upgrade the major version of `MariaDB` standalone.
...
The above `spec.upgradeConstraints.denylist` is showing that upgrading below version of `5.7.31` is not possible for both standalone and group replication. That means, it is possible to upgrade any version above `5.7.31`. Here, we are going to create a `MariaDB` standalone using MariaDB  `5.7.31`. Then we are going to upgrade this version to `8.0.21`.

#### Prepare Standalone

Now, we are going to deploy a `MariaDB` standalone using version `5.7.31`.
...
```bash
$ kubectl create -f https://github.com/kubedb/docs/raw/{{< param "info.version" >}}/docs/examples/mariadb/upgrading/majorversin/standalone.yaml
...
Here, we are going to upgrade `MariaDB` standalone from `5.7.31` to `8.0.21`.
...
```bash
$ kubectl apply -f https://github.com/kubedb/docs/raw/{{< param "info.version" >}}/docs/examples/mariadb/upgrading/majorversion/upgrade_major_version_standalone.yaml
mariadbopsrequest.ops.kubedb.com/my-upgrade-major-standalone created
```
...
Metadata:
  ...
  Self Link:         /apis/ops.kubedb.com/v1alpha1/namespaces/demo/mariadbopsrequests/my-upgrade-major-standalone
...
Status:
  Conditions:
    ...
    Message:               Controller has started to Progress the MariaDBOpsRequest: demo/my-upgrade-major-standalone
    ...
    Type:                  HaltDatabase
    Last Transition Time:  2020-08-12T10:08:06Z
    Message:               MariaDB version upgrading stated for MariaDBOpsRequest: demo/my-upgrade-major-standalone
    Observed Generation:   1
    Reason:                DatabaseVersionUpgradingStarted
    Status:                True
    Type:                  Upgrading
    Last Transition Time:  2020-08-12T10:10:06Z
    Message:               Image successfully updated in MariaDB: demo/my-standalone for MariaDBOpsRequest: my-upgrade-major-standalone
    Observed Generation:   1
    ...
    Type:                  ResumeDatabase
    Last Transition Time:  2020-08-12T10:10:06Z
    Message:               Controller has successfully scaled/upgraded the MariaDB demo/my-upgrade-major-standalone
    Observed Generation:   2
...
Events:
  ...
  ----    ------      ----   ----                        -------
  Normal  Starting    4m48s  KubeDB Enterprise Operator  Start processing for MariaDBOpsRequest: demo/my-upgrade-major-standalone
  Normal  Starting    4m48s  KubeDB Enterprise Operator  Pausing MariaDB databse: demo/my-standalone
  Normal  Successful  4m48s  KubeDB Enterprise Operator  Successfully halted MariaDB database: demo/my-standalone for MariaDBOpsRequest: my-upgrade-major-standalone
  Normal  Starting    4m48s  KubeDB Enterprise Operator  Upgrading MariaDB images: demo/my-standalone for MariaDBOpsRequest: my-upgrade-major-standalone
  Normal  Successful  3m8s   KubeDB Enterprise Operator  Image successfully upgraded for standalone/master: demo/my-standalone-0
  Normal  Successful  2m48s  KubeDB Enterprise Operator  Image successfully updated in MariaDB: demo/my-standalone for MariaDBOpsRequest: my-upgrade-major-standalone

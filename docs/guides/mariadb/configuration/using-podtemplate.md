...
KubeDB supports providing custom configuration for MariaDB via [PodTemplate](/docs/guides/mariadb/concepts/mariadb.md#specpodtemplate). This tutorial will show you how to use KubeDB to run a MariaDB database with custom configuration using PodTemplate.
...
Read about the fields in details in [PodTemplate concept](/docs/guides/mariadb/concepts/mariadb.md#specpodtemplate),

## CRD Configuration

Below is the YAML for the MariaDB created in this example. Here, [`spec.podTemplate.spec.env`](/docs/guides/mariadb/concepts/mariadb.md#specpodtemplatespecenv) specifies environment variables and [`spec.podTemplate.spec.args`](/docs/guides/mariadb/concepts/mariadb.md#specpodtemplatespecargs) provides extra arguments for [MariaDB Docker Image](https://hub.docker.com/_/mariadb/).
...
```bash
$ kubectl create -f https://github.com/kubedb/docs/raw/{{< param "info.version" >}}/docs/examples/mariadb/configuration/mariadb-misc-config.yaml
mariadb.kubedb.com/mariadb-misc-config created
```

Now, wait a few minutes. KubeDB operator will create necessary PVC, statefulset, services, secret etc. If everything goes well, we will see that a pod with the name `mariadb-misc-config-0` has been created.

Check that the statefulset's pod is running

```bash
$ kubectl get pod -n demo -l app.kubernetes.io/name=mariadbs.kubedb.com,app.kubernetes.io/instance=mariadb-misc-config
NAME                  READY   STATUS    RESTARTS   AGE
mariadb-misc-config-0   1/1     Running   0          9m28s
```

Check the pod's log to see if the database is ready

```bash
$ kubectl logs -f -n demo mariadb-misc-config-0
...
....
2018-10-02T09:34:33.694994Z 0 [Note] mariadbd: ready for connections.
Version: '5.7.31'  socket: '/var/run/mariadbd/mariadbd.sock'  port: 3306  MariaDB Community Server (GPL)
....
```
...
```bash
$ kubectl get pods mariadb-misc-config-0 -n demo -o yaml | grep IP
  ...
  hostIP: 10.0.2.15
  podIP: 172.17.0.6

$ kubectl get secrets -n demo mariadb-misc-config-auth -o jsonpath='{.data.\user}' | base64 -d
root

$ kubectl get secrets -n demo mariadb-misc-config-auth -o jsonpath='{.data.\password}' | base64 -d
MLO5_fPVKcqPiEu9
```
...
![mariadb_all_databases](/docs/images/mariadb/mariadb-all-databases.png)

![mariadb_charset](/docs/images/mariadb/mariadb-charset.png)
...
```bash
kubectl patch -n demo my/mariadb-misc-config -p '{"spec":{"terminationPolicy":"WipeOut"}}' --type="merge"
kubectl delete -n demo my/mariadb-misc-config

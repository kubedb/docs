...
MariaDB allows to configure database via configuration file. The default configuration for MariaDB can be found in `/etc/mariadb/my.cnf` file. When MariaDB starts, it will look for custom configuration file in `/etc/mariadb/conf.d` directory. If configuration file exist, MariaDB instance will use combined startup setting from both `/etc/mariadb/my.cnf` and `*.cnf` files in `/etc/mariadb/conf.d` directory. This custom configuration will overwrite the existing default one. To know more about configuring MariaDB see [here](https://dev.mariadb.com/doc/refman/8.0/en/server-configuration.html).

At first, you have to create a config file with `.cnf` extension with your desired configuration. Then you have to put this file into a [volume](https://kubernetes.io/docs/concepts/storage/volumes/). You have to specify this volume  in `spec.configSecret` section while creating MariaDB crd. KubeDB will mount this volume into `/etc/mariadb/conf.d` directory of the database pod.

In this tutorial, we will configure [max_connections](https://dev.mariadb.com/doc/refman/8.0/en/server-system-variables.html#sysvar_max_connections) and [read_buffer_size](https://dev.mariadb.com/doc/refman/8.0/en/server-system-variables.html#sysvar_read_buffer_size) via a custom config file. We will use configMap as volume source.
...
```bash
$ kubectl apply -f https://github.com/kubedb/docs/raw/{{< param "info.version" >}}/docs/examples/mariadb/configuration/mariadb-custom.yaml
mariadb.kubedb.com/custom-mariadb created
```
...
Now, wait a few minutes. KubeDB operator will create necessary PVC, statefulset, services, secret etc. If everything goes well, we will see that a pod with the name `custom-mariadb-0` has been created.
...
custom-mariadb-0   1/1       Running   0          44s
```

Check the pod's log to see if the database is ready

```bash
$ kubectl logs -f -n demo custom-mariadb-0
...
2020-08-26T10:45:28.573533Z 0 [System] [MY-013602] [Server] Channel mariadb_main configured to support TLS. Encrypted connections are now supported for this channel.
...
2020-08-26T10:45:28.610887Z 0 [System] [MY-010931] [Server] /usr/sbin/mariadbd: ready for connections. Version: '8.0.21'  socket: '/var/run/mariadbd/mariadbd.sock'  port: 3306  MariaDB Community Server - GPL.
....
```

Once we see `[Note] /usr/sbin/mariadbd: ready for connections.` in the log, the database is ready.
...
```bash
$ kubectl get pods custom-mariadb-0 -n demo -o yaml | grep IP
  hostIP: 10.0.2.15
  podIP: 172.17.0.6

$ kubectl get secrets -n demo custom-mariadb-auth -o jsonpath='{.data.\user}' | base64 -d
root

$ kubectl get secrets -n demo custom-mariadb-auth -o jsonpath='{.data.\password}' | base64 -d
MLO5_fPVKcqPiEu9
```
...
![max_connections](/docs/images/mariadb/max_connection.png)

![read_buffer_size](/docs/images/mariadb/read_buffer_size.png)
...
```bash
kubectl patch -n demo my/custom-mariadb -p '{"spec":{"terminationPolicy":"WipeOut"}}' --type="merge"
kubectl delete -n demo my/custom-mariadb

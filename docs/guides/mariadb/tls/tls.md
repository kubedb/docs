...
```bash
kubectl apply -f ./docs/examples/my/mariadb/issuer.yaml
issuer.cert-manager.io/mariadb-issuer created
```

### Deploy MariaDB Standalone with TLS/SSL configuration

Here, our issuer `mariadb-issuer`  is ready to deploy a `MariaDB` standalone with TLS/SSL configuration. Below is the YAML for MariaDB Standalone that we are going to create,
...
- `spec.tls.issuerRef` refers to the `mariadb-issuer` issuer.
...
You can found more details from [here](/docs/guides/mariadb/concepts/mariadb.md#tls)
...
```bash
$ kubectl apply -f https://github.com/kubedb/docs/raw/{{< param "info.version" >}}/docs/examples/mariadb/tls/tls-standalone.yaml
mariadb.kubedb.com/my-standalone-tls created
```
...
$ watch -n 3 kubectl get pod -n demo -l app.kubernetes.io/name=mariadbs.kubedb.com,app.kubernetes.io/instance=my-standalone-tls
Every 3.0s: kubectl get pod -n demo -l app.kubernetes.io/name=mariadbs.kubedb.com...  suaas-appscode: Thu Aug 13 18:13:19 2020
...
| mariadbx_ssl_cipher  |                             |
| mariadbx_ssl_crl     |                             |
| mariadbx_ssl_crlpath |                             |
| mariadbx_ssl_key     |                             |
| ssl_ca             | /etc/mariadb/certs/ca.crt     |
| ssl_capath         | /etc/mariadb/certs            |
| ssl_cert           | /etc/mariadb/certs/server.crt |
| ssl_cipher         |                             |
| ssl_crl            |                             |
| ssl_crlpath        |                             |
| ssl_fips_mode      | OFF                         |
| ssl_key            | /etc/mariadb/certs/server.key |
+--------------------+-----------------------------+
24 rows in set (0.00 sec)

mariadb> SHOW VARIABLES LIKE '%require_secure_transport%';
+--------------------------+-------+
| Variable_name            | Value |
+--------------------------+-------+
...
The above output shows that the `MariaDB` server is configured to TLS/SSL. You can also see that the `.crt` and `.key` files are stored in the `/etc/ mariadb/certs/` directory for client and server.
...
root@mariadb-tls-0:/# mariadb -uroot -p${MYSQL_ROOT_PASSWORD}
mariadb: [Warning] Using a password on the command line interface can be insecure.
Welcome to the MariaDB monitor.  Commands end with ; or \g.
...
mariadb> CREATE USER 'mariadb_user'@'localhost' IDENTIFIED BY 'pass' REQUIRE SSL;
...
root@mariadb-tls-0:/# mariadb -umariadb_user -ppass
mariadb: [Warning] Using a password on the command line interface can be insecure.
ERROR 1045 (28000): Access denied for user 'mariadb_user'@'localhost' (using password: YES)

# accessing the database server newly created user with ssl-mode=disable
root@mariadb-tls-0:/# mariadb -umariadb_user -ppass --ssl-mode=disabled
mariadb: [Warning] Using a password on the command line interface can be insecure.
ERROR 1045 (28000): Access denied for user 'mariadb_user'@'localhost' (using password: YES)

# accessing the database server newly created user with certificates
root@mariadb-tls-0:/# mariadb -umariadb_user -ppass --ssl-ca=/etc/mariadb/certs/ca.crt  --ssl-cert=/etc/mariadb/certs/client.crt --ssl-key=/etc/mariadb/certs/client.key
mariadb: [Warning] Using a password on the command line interface can be insecure.
Welcome to the MariaDB monitor.  Commands end with ; or \g.
...
From the above output, you can see that only using client certificate we can access the database securely, otherwise, it shows "Access denied". Our client certificate is stored in `/etc/mariadb/certs/` directory.
...
```bash
kubectl apply -f https://github.com/kubedb/docs/raw/{{< param "info.version" >}}/docs/examples/mariadb/tls/tls-group.yaml
mariadb.kubedb.com/my-group-tls created
```
...
$ watch -n 3 kubectl get pod -n demo -l app.kubernetes.io/name=mariadbs.kubedb.com,app.kubernetes.io/instance=my-group-tls
Every 3.0s: kubectl get pod -n demo -l app.kubernetes.io/name=mariadbs.kubedb.com  suaas-appscode: Thu Aug 13 19:03:02 2020
...
```bash
$ kubectl exec -it -n  demo  my-group-tls-0 -c mariadb -- bash
root@my-group-0:/# ls /etc/mariadb/certs/
ca.crt  client.crt  client.key  server.crt  server.key

root@my-group-0:/# mariadb -u${MYSQL_ROOT_USERNAME} -p${MYSQL_ROOT_PASSWORD}
mariadb: [Warning] Using a password on the command line interface can be insecure.
Welcome to the MariaDB monitor.  Commands end with ; or \g.
...
root@my-group-0:/# mariadb -uroot -p${MYSQL_ROOT_PASSWORD}
mariadb: [Warning] Using a password on the command line interface can be insecure.
Welcome to the MariaDB monitor.  Commands end with ; or \g.
...
root@my-group-0:/# mariadb -umariadb_user -ppass
mariadb: [Warning] Using a password on the command line interface can be insecure.
ERROR 1045 (28000): Access denied for user 'mariadb_user'@'localhost' (using password: YES)

# accessing the database server newly created user with ssl-mode=disable
root@my-group-0:/# mariadb -umariadb_user -ppass --ssl-mode=disabled
mariadb: [Warning] Using a password on the command line interface can be insecure.
ERROR 1045 (28000): Access denied for user 'mariadb_user'@'localhost' (using password: YES)

# accessing the database server newly created user with certificates
root@my-group-0:/# mariadb -umariadb_user -ppass --ssl-ca=/etc/mariadb/certs/ca.crt  --ssl-cert=/etc/mariadb/certs/client.crt --ssl-key=/etc/mariadb/certs/client.key
mariadb: [Warning] Using a password on the command line interface can be insecure.
Welcome to the MariaDB monitor.  Commands end with ; or \g.

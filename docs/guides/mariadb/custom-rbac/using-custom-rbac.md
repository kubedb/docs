...
Now, we need to create a role that has necessary access permissions for the MariaDB instance named `quick-mariadb`.

```bash
$ kubectl apply -f https://github.com/kubedb/docs/raw/{{< param "info.version" >}}/docs/examples/mariadb/custom-rbac/my-custom-role.yaml
...
```bash
$ kubectl apply -f https://github.com/kubedb/docs/raw/{{< param "info.version" >}}/docs/examples/mariadb/custom-rbac/my-custom-db.yaml
mariadb.kubedb.com/quick-mariadb created
```
...
Now, wait a few minutes. the KubeDB operator will create necessary PVC, StatefulSet, services, secret etc. If everything goes well, we should see that a pod with the name `quick-mariadb-0` has been created.

Check that the statefulset's pod is running

```bash
$ kubectl get pod -n demo quick-mariadb-0
NAME            READY   STATUS    RESTARTS   AGE
quick-mariadb-0   1/1     Running   0          2m44s
```

Check the pod's log to see if the database is ready

```bash
$ kubectl logs -f -n demo quick-mariadb-0
...
2020-08-27 06:01:50+00:00 [Note] [Entrypoint]: MariaDB init process done. Ready for start up.

2020-08-27T06:01:51.142462Z 0 [System] [MY-010116] [Server] /usr/sbin/mariadbd (mariadbd 8.0.21) starting as process 1
2020-08-27T06:01:51.142509Z 0 [ERROR] [MY-010338] [Server] Can't find error-message file '/usr/share/mariadb-8.0/errmsg.sys'. Check error-message file location and 'lc-messages-dir' configuration directive.
...
2020-08-27T06:01:51.924256Z 0 [System] [MY-013602] [Server] Channel mariadb_main configured to support TLS. Encrypted connections are now supported for this channel.
2020-08-27T06:01:51.931573Z 0 [Warning] [MY-011810] [Server] Insecure configuration for --pid-file: Location '/var/run/mariadbd' in the path is accessible to all OS users. Consider choosing a different directory.
2020-08-27T06:01:51.955689Z 0 [System] [MY-010931] [Server] /usr/sbin/mariadbd: ready for connections. Version: '8.0.21'  socket: '/var/run/mariadbd/mariadbd.sock'  port: 3306  MariaDB Community Server - GPL.
```
...
Now, create MariaDB crd `minute-mariadb` using the existing service account name `my-custom-serviceaccount` in the `spec.podTemplate.spec.serviceAccountName` field.

```bash
$ kubectl apply -f https://github.com/kubedb/docs/raw/{{< param "info.version" >}}/docs/examples/mariadb/custom-rbac/my-custom-db-two.yaml
mariadb.kubedb.com/quick-mariadb created
```
...
Now, wait a few minutes. the KubeDB operator will create necessary PVC, statefulset, services, secret etc. If everything goes well, we should see that a pod with the name `minute-mariadb-0` has been created.

Check that the statefulset's pod is running

```bash
$ kubectl get pod -n demo minute-mariadb-0
NAME             READY     STATUS    RESTARTS   AGE
minute-mariadb-0   1/1       Running   0          14m
```
...
```bash
kubectl patch -n demo my/quick-mariadb -p '{"spec":{"terminationPolicy":"WipeOut"}}' --type="merge"
kubectl delete -n demo my/quick-mariadb

kubectl patch -n demo my/minute-mariadb -p '{"spec":{"terminationPolicy":"WipeOut"}}' --type="merge"
kubectl delete -n demo my/minute-mariadb
...
- Monitor your MariaDB instance with KubeDB using [out-of-the-box Prometheus operator](/docs/guides/mariadb/monitoring/using-prometheus-operator.md).
- Monitor your MariaDB instance with KubeDB using [out-of-the-box builtin-Prometheus](/docs/guides/mariadb/monitoring/using-builtin-prometheus.md).

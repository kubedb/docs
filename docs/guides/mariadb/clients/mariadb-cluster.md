---
title: Connecting to a MariaDB Cluster
menu:
  docs_{{ .version }}:
    identifier: my-mariadb-cluster-clients
    name: Connecting to a MariaDB Cluster
    parent: my-clients-mariadb
    weight: 10
...
# Connecting to a MariaDB Cluster
...
- You need to have a mariadb client. If you don't have a mariadb client install in your local machine, you can install from [here](https://dev.mariadb.com/doc/mariadb-installation-excerpt/8.0/en/)
...
Here, we are going to deploy a `MariaDB` group replication using a supported version by KubeDB operator. Then we are going to connect with the cluster using two separate services.

Below is the YAML of `MariaDB` group replication with 3 members (one is a primary member and the two others are secondary members) that we are going to create.
...
KubeDB operator watches for `MariaDB` objects using Kubernetes API. When a `MariaDB` object is created, KubeDB operator will create a new StatefulSet and two separate Services for client connection with the cluster. The services have the following format:

- `<mariadb-object-name>` has both read and write operation.
- `<mariadb-object-name>-replicas` has only read operation.

Now, wait for the `MariaDB` is going to `Running` state and also wait for `SatefulSet` and `services` going to the `Ready` state.
...
Events:
  ...
  ----    ------      ----  ----            -------
  Normal  Successful  8m    MariaDB operator  Successfully created Service
  Normal  Successful  8m    MariaDB operator  Successfully created service for secondary replicas
  Normal  Successful  3m    MariaDB operator  Successfully created StatefulSet
  Normal  Successful  3m    MariaDB operator  Successfully created MariaDB
  Normal  Successful  3m    MariaDB operator  Successfully created appbinding
```
...
- `mariadb.kubedb.com/role:primary` are added for primary.
- `mariadb.kubedb.com/role:secondary` are added for secondary.

Let's verify that the `mariadb.kubedb.com/role:<primary/secondary>` label are added into the StatefulSet's replicas,

```bash
$ kubectl get pods -n demo -l app.kubernetes.io/name=mariadbs.kubedb.com,app.kubernetes.io/instance=my-group -A -o=custom-columns='Name:.metadata.name,Labels:metadata.labels,PodIP:.status.podIP'
Name         Labels                                                                                                                                                                           PodIP
my-group-0   map[controller-revision-hash:my-group-55b9f49f98 app.kubernetes.io/name:mariadbs.kubedb.com app.kubernetes.io/instance:my-group mariadb.kubedb.com/role:primary statefulset.kubernetes.io/pod-name:my-group-0]     10.244.1.8
my-group-1   map[controller-revision-hash:my-group-55b9f49f98 app.kubernetes.io/name:mariadbs.kubedb.com app.kubernetes.io/instance:my-group mariadb.kubedb.com/role:secondary statefulset.kubernetes.io/pod-name:my-group-1]   10.244.2.11
my-group-2   map[controller-revision-hash:my-group-55b9f49f98 app.kubernetes.io/name:mariadbs.kubedb.com app.kubernetes.io/instance:my-group mariadb.kubedb.com/role:secondary statefulset.kubernetes.io/pod-name:my-group-2]   10.244.2.13
```

You can see from the above output that the `my-group-0` pod is selected as a primary member in our existing database cluster. It has the `mariadb.kubedb.com/role:primary` label and the podIP is `10.244.1.8`. Besides, the rest of the replicas are selected as a secondary member which has `mariadb.kubedb.com/role:secondary` label.
...
$ kubectl get svc -n demo my-group -o json | jq '.spec.selector'
{
  "app.kubernetes.io/name": "mariadbs.kubedb.com",
  "app.kubernetes.io/instance": "my-group",
  "mariadb.kubedb.com/role": "primary"
}
```
...
$ kubectl get svc -n demo my-group-replicas -o json | jq '.spec.selector'
{
  "app.kubernetes.io/name": "mariadbs.kubedb.com",
  "app.kubernetes.io/instance": "my-group",
  "mariadb.kubedb.com/role": "secondary"
}
```
...
>For testing purpose, we need to have a mariadb client to connect with the cluster. If you don't have a client in your local machine, you can install from [here](https://dev.mariadb.com/doc/mariadb-installation-excerpt/8.0/en/)

**Write Operation :**

```bash
# create a database on cluster
$ mariadb -uroot -pRmxLjEomvE6tVj4- --port=8081 --host=127.0.0.1 -e "CREATE DATABASE playground;"
mariadb: [Warning] Using a password on the command line interface can be insecure.


# create a table
$ mariadb -uroot -pRmxLjEomvE6tVj4- --port=8081 --host=127.0.0.1 -e "CREATE TABLE playground.equipment ( id INT NOT NULL AUTO_INCREMENT, type VARCHAR(50), quant INT, color VARCHAR(25), PRIMARY KEY(id));"
mariadb: [Warning] Using a password on the command line interface can be insecure.


# insert a row
$ mariadb -uroot -pRmxLjEomvE6tVj4- --port=8081 --host=127.0.0.1 -e "INSERT INTO playground.equipment (type, quant, color) VALUES ('slide', 2, 'blue');"
mariadb: [Warning] Using a password on the command line interface can be insecure.
```
...
$ mariadb -uroot -pRmxLjEomvE6tVj4- --port=8081 --host=127.0.0.1 -e "SELECT * FROM playground.equipment;"
mariadb: [Warning] Using a password on the command line interface can be insecure.
+----+-------+-------+-------+
| id | type  | quant | color |
+----+-------+-------+-------+
...
# insert a row
$ mariadb -uroot -pRmxLjEomvE6tVj4- --port=8080 --host=127.0.0.1 -e "INSERT INTO playground.equipment (type, quant, color) VALUES ('slide', 3, 'black');"
mariadb: [Warning] Using a password on the command line interface can be insecure.
...
```bash
# read data from cluster
$ mariadb -uroot -pRmxLjEomvE6tVj4- --port=8080 --host=127.0.0.1 -e "SELECT * FROM playground.equipment;"
mariadb: [Warning] Using a password on the command line interface can be insecure.
+----+-------+-------+-------+
| id | type  | quant | color |
+----+-------+-------+-------+
...
$ kubectl get pods -n demo -l app.kubernetes.io/name=mariadbs.kubedb.com,app.kubernetes.io/instance=my-group -A -o=custom-columns='Name:.metadata.name,Labels:metadata.labels,PodIP:.status.podIP'
Name         Labels                                                                                                                                                                           PodIP
my-group-0   map[controller-revision-hash:my-group-55b9f49f98 app.kubernetes.io/name:mariadbs.kubedb.com app.kubernetes.io/instance:my-group mariadb.kubedb.com/role:secondary statefulset.kubernetes.io/pod-name:my-group-0]   10.244.2.18
my-group-1   map[controller-revision-hash:my-group-55b9f49f98 app.kubernetes.io/name:mariadbs.kubedb.com app.kubernetes.io/instance:my-group mariadb.kubedb.com/role:secondary statefulset.kubernetes.io/pod-name:my-group-1]   10.244.2.11
my-group-2   map[controller-revision-hash:my-group-55b9f49f98 app.kubernetes.io/name:mariadbs.kubedb.com app.kubernetes.io/instance:my-group mariadb.kubedb.com/role:primary statefulset.kubernetes.io/pod-name:my-group-2]     10.244.2.13
```

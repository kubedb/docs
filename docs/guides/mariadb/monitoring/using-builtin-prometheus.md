...
```bash
$ kubectl apply -f https://github.com/kubedb/docs/raw/{{< param "info.version" >}}/docs/examples/mariadb/monitoring/builtin-prom-mariadb.yaml
mariadb.kubedb.com/builtin-prom-mariadb created
```

Now, wait for the database to go into `Running` state.

```bash
$ watch -n 3 kubectl get mariadb -n demo builtin-prom-mariadb
Every 3.0s: kubectl get mariadb -n demo builtin-prom-mariadb        suaas-appscode: Tue Aug 25 16:07:29 2020

NAME                 VERSION   STATUS    AGE
builtin-prom-mariadb   8.0.21    Running   3m33s
```

KubeDB will create a separate stats service with name `{MariaDB crd name}-stats` for monitoring purpose.

```bash
$ kubectl get svc -n demo --selector="app.kubernetes.io/instance=builtin-prom-mariadb"
NAME                       TYPE        CLUSTER-IP      EXTERNAL-IP   PORT(S)     AGE
builtin-prom-mariadb         ClusterIP   10.104.141.73   <none>        3306/TCP    4m45s
builtin-prom-mariadb-gvr     ClusterIP   None            <none>        3306/TCP    4m45s
builtin-prom-mariadb-stats   ClusterIP   10.103.209.43   <none>        56790/TCP   2m9s
```

Here, `builtin-prom-mariadb-stats` service has been created for monitoring purpose. Let's describe the service.

```bash
$ kubectl describe svc -n demo builtin-prom-mariadb-stats
Name:              builtin-prom-mariadb-stats
Namespace:         demo
Labels:            app.kubernetes.io/name=mariadbs.kubedb.com
                   app.kubernetes.io/instance=builtin-prom-mariadb
                   kubedb.com/role=stats
Annotations:       monitoring.appscode.com/agent: prometheus.io/builtin
                   prometheus.io/path: /metrics
                   prometheus.io/port: 56790
                   prometheus.io/scrape: true
Selector:          app.kubernetes.io/name=mariadbs.kubedb.com,app.kubernetes.io/instance=builtin-prom-mariadb
Type:              ClusterIP
IP:                10.103.209.43
...
Now, we can access the dashboard at `localhost:9090`. Open [http://localhost:9090](http://localhost:9090) in your browser. You should see the endpoint of `builtin-prom-mariadb-stats` service as one of the targets.

<p align="center">
  <img alt="Prometheus Target" height="100%" src="/docs/images/mariadb/monitoring/mariadb-builtin-prom-target.png" style="padding:10px">
</p>

Check the labels marked with red rectangle. These labels confirm that the metrics are coming from `MariaDB` database `builtin-prom-mariadb` through stats service `builtin-prom-mariadb-stats`.
...
```bash
kubectl delete -n demo my/builtin-prom-mariadb

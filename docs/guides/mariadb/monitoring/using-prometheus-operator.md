...
- To learn how Prometheus monitoring works with KubeDB in general, please visit [here](/docs/guides/mariadb/monitoring/overview.md).
...
mariadb.kubedb.com/coreos-prom-mariadb created
```

Now, wait for the database to go into `Running` state.

```bash
$ watch -n 3 kubectl get mariadb -n demo coreos-prom-mariadb
Every 3.0s: kubectl get mariadb -n demo coreos-prom-mariadb         suaas-appscode: Tue Aug 25 11:53:34 2020

NAME                VERSION   STATUS    AGE
coreos-prom-mariadb   8.0.21    Running   2m53s
```

KubeDB will create a separate stats service with name `{MariaDB crd name}-stats` for monitoring purpose.

```bash
$ kubectl get svc -n demo --selector="app.kubernetes.io/instance=coreos-prom-mariadb"
NAME                      TYPE        CLUSTER-IP       EXTERNAL-IP   PORT(S)     AGE
coreos-prom-mariadb         ClusterIP   10.103.228.135   <none>        3306/TCP    3m36s
coreos-prom-mariadb-gvr     ClusterIP   None             <none>        3306/TCP    3m36s
coreos-prom-mariadb-stats   ClusterIP   10.106.236.14    <none>        56790/TCP   50s
```

Here, `coreos-prom-mariadb-stats` service has been created for monitoring purpose.

Let's describe this stats service.

```yaml
$ kubectl describe svc -n demo coreos-prom-mariadb-stats
Name:              coreos-prom-mariadb-stats
Namespace:         demo
Labels:            app.kubernetes.io/name=mariadbs.kubedb.com
                   app.kubernetes.io/instance=coreos-prom-mariadb
                   kubedb.com/role=stats
Annotations:       monitoring.appscode.com/agent: prometheus.io/operator
Selector:          app.kubernetes.io/name=mariadbs.kubedb.com,app.kubernetes.io/instance=coreos-prom-mariadb
Type:              ClusterIP
IP:                10.106.236.14
...
KubeDB will also create a `ServiceMonitor` crd in `demo` namespace that select the endpoints of `coreos-prom-mariadb-stats` service. Verify that the `ServiceMonitor` crd has been created.

```bash
$ kubectl get servicemonitor -n demo
NAME                            AGE
kubedb-demo-coreos-prom-mariadb   3m16s
```

Let's verify that the `ServiceMonitor` has the label that we had specified in `spec.monitor` section of MariaDB crd.

```yaml
$ kubectl get servicemonitor -n demo kubedb-demo-coreos-prom-mariadb -o yaml
apiVersion: monitoring.coreos.com/v1
kind: ServiceMonitor
metadata:
  ...
  ...
  name: kubedb-demo-coreos-prom-mariadb
  namespace: demo
  ownerReferences:
  - apiVersion: v1
  ...
  selfLink: /apis/monitoring.coreos.com/v1/namespaces/demo/servicemonitors/kubedb-demo-coreos-prom-mariadb
...
Also notice that the `ServiceMonitor` has selector which match the labels we have seen in the `coreos-prom-mariadb-stats` service. It also, target the `prom-http` port that we have seen in the stats service.
...
Now, we can access the dashboard at `localhost:9090`. Open [http://localhost:9090](http://localhost:9090) in your browser. You should see `prom-http` endpoint of `coreos-prom-mariadb-stats` service as one of the targets.

<p align="center">
  <img alt="Prometheus Target" src="/docs/images/mariadb/monitoring/mariadb-coreos-prom-target.png" style="padding:10px">
</p>
...
```bash
# cleanup database
kubectl delete -n demo my/coreos-prom-mariadb

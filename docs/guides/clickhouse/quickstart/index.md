...
root@clickhouse-quickstart-0:/# clickhouse -uroot -p"H(Y.s)pg&cX1Ds3J"

Welcome to the ClickHouse monitor.  Commands end with ; or \g.
Your ClickHouse connection id is 351
Server version: 8.0.35 ClickHouse Community Server - GPL
...
```bash
$ kubectl create -f https://github.com/kubedb/docs/raw/{{< param "info.version" >}}/docs/guides/clickhouse/quickstart/yamls/phpmyadmin.yaml
...
clickhouse-quickstart-0         1/1     Running   0          12m


$ kubectl get svc -n demo
NAME                    TYPE           CLUSTER-IP      EXTERNAL-IP   PORT(S)        AGE
myadmin                 LoadBalancer   10.96.108.199   <pending>     80:32634/TCP   51s
clickhouse-quickstart        ClusterIP      10.96.150.194   <none>        3306/TCP       13m
clickhouse-quickstart-pods   ClusterIP      None            <none>        3306/TCP       13m
...
>Note: In ClickHouse: `8.0.14` connection to phpMyAdmin may give error as it is using `caching_sha2_password` and `sha256_password` authentication plugins over `clickhouse_native_password`. If the error happens do the following for work around. But, It's not recommended to change authentication plugins. See [here](https://stackoverflow.com/questions/49948350/phpmyadmin-on-clickhouse-8-0) for alternative solutions. You can use clickhouse_native_password try `kubectl exec -it -n demo clickhouse-quickstart-0 -- clickhouse -u root --password='H(Y.s)pg&cX1Ds3J' -e "ALTER USER root IDENTIFIED WITH clickhouse_native_password BY 'H(Y.s)pg&cX1Ds3J';"`
---
To log into the phpMyAdmin, use host __`clickhouse-quickstart.demo`__ or __`10.244.0.30`__ , username __`root`__ and password __`H(Y.s)pg&cX1Ds3J`__.

__connecting with `dbgate`__


```bash
$ kubectl create -f https://github.com/kubedb/docs/raw/{{< param "info.version" >}}/docs/guides/clickhouse/quickstart/yamls/dbgate.yaml
...
You can connect multiple different database using db gate. To log into  ClickHouse select the MYSQL driver and use server __`clickhouse-quickstart.demo`__ or __`10.244.0.30`__ , username __`root`__ and password __`H(Y.s)pg&cX1Ds3J`__.
...
Learn details of all `DeletionPolicy` [here](/docs/guides/clickhouse/concepts/database/index.md#specdeletionpolicy).
...
At first, run `kubectl edit my clickhouse-quickstart -n demo` to set `spec.deletionPolicy` to `Halt`. Then delete the clickhouse object,
...
- Initialize [ClickHouse with Script](/docs/guides/clickhouse/initialization/index.md).
- Monitor your ClickHouse database with KubeDB using [out-of-the-box Prometheus operator](/docs/guides/clickhouse/monitoring/prometheus-operator/index.md).
- Monitor your ClickHouse database with KubeDB using [out-of-the-box builtin-Prometheus](/docs/guides/clickhouse/monitoring/builtin-prometheus/index.md).
- Use [private Docker registry](/docs/guides/clickhouse/private-registry/index.md) to deploy ClickHouse with KubeDB.
- Detail concepts of [ClickHouse object](/docs/guides/clickhouse/concepts/database/index.md).
- Detail concepts of [ClickHouseVersion object](/docs/guides/clickhouse/concepts/catalog/index.md).

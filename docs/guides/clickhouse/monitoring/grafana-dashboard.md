---
title: Visualize ClickHouse Metrics with Grafana Dashboard
menu:
  docs_{{ .version }}:
...
> These two steps — deploying `kube-prometheus-stack` and installing Panopticon — are shared prerequisites for all KubeDB database monitoring guides. If you have already completed them in another guide, skip to [Step 1](#step-1-deploy-clickhouse-with-monitoring-enabled).
...
<p align="center">
  <img alt="Prometheus Target" src="/docs/images/clickhouse/monitoring/ch-prom-targets.png" style="padding:10px">
</p>
...
<p align="center">
  <img alt="Grafana Login" src="/docs/images/clickhouse/monitoring/ch-grafana-login.png" style="padding:10px">
</p>

After a successful login you will see the Grafana home page:

<p align="center">
  <img alt="Grafana Home" src="/docs/images/clickhouse/monitoring/ch-grafana-home.png" style="padding:10px">
</p>
...
## Step 5: Import KubeDB ClickHouse Dashboard — Option A: Automatically, via the `kubedb-grafana-dashboards` chart
...
`featureGates.ClickHouse` already defaults to `true` — set explicitly above for clarity. `grafana.name`/`grafana.namespace` point the chart's `GrafanaDashboard` resources at the `AppBinding` created in step 2 (omitting them falls back to whichever `AppBinding` in your cluster is labeled as the cluster-default Grafana, if any — explicit is safer on a shared cluster).

This single command creates every dashboard this chart ships for ClickHouse — `KubeDB / ClickHouse / Summary`, `KubeDB / ClickHouse / Pod`, `KubeDB / ClickHouse / Database` — which `grafana-operator` then pushes into your Grafana instance automatically. No manual JSON download or upload needed.
...
## Step 5: Import KubeDB ClickHouse Dashboard — Option B: Manually, by uploading JSON files
...
<p align="center">
  <img alt="Grafana Import Dashboard" src="/docs/images/clickhouse/monitoring/ch-grafana-import.png" style="padding:10px">
</p>

...
Three dashboards are available. Download all three JSON files from the [opnpulse/dashboards](https://github.com/opnpulse/dashboards/tree/master/postgres) repository (`postgres/` folder):
...
| **dataname** | Database dashboard only | A specific database inside Postgres, or `All` |

...
### Step 1: Deploy PostgreSQL with Monitoring Enabled
...
## Step 5: Import KubeDB PostgreSQL Dashboard — Option A: Automatically, via the `kubedb-grafana-dashboards` chart
...
`featureGates.Postgres` already defaults to `true` — set explicitly above for clarity. `grafana.name`/`grafana.namespace` point the chart's `GrafanaDashboard` resources at the `AppBinding` created in step 2 (omitting them falls back to whichever `AppBinding` in your cluster is labeled as the cluster-default Grafana, if any — explicit is safer on a shared cluster).

This single command creates every dashboard this chart ships for PostgreSQL — `KubeDB / Postgres / Summary`, `KubeDB / Postgres / Pod`, `KubeDB / Postgres / Database` — which `grafana-operator` then pushes into your Grafana instance automatically. No manual JSON download or upload needed.
...
## Step 5: Import KubeDB PostgreSQL Dashboard — Option B: Manually, by uploading JSON files

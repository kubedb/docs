...
Panopticon is the Appscode operator that exports the KubeDB operator's own view of every resource — `kubedb_com_postgres_status_phase` and related metrics. It's what powers the **Provisioner Group** alerts below (`KubeDBPostgresPhaseNotReady`/`KubeDBPostgresPhaseCritical`). Skip this step if you only need the exporter-based **Database Group** alerts.

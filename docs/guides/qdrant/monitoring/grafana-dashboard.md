...
## Step 3: Deploy Qdrant with Monitoring Enabled
...
Open [http://localhost:9090/targets](http://localhost:9090/targets) in your browser. Look for an entry whose `service` label matches `qdrant-grafana-demo-stats`. Its state should be **UP**.
...
## Step 7: Import KubeDB Qdrant Dashboards
...
- **Collections Count** — number of vector collections
- **Vectors Indexed** — total vectors stored across all collections
- **Search Latency** — p50/p99 search request latency
...
**KubeDB / Qdrant / Pod** — per-pod drill-down:
- **Uptime** — how long this pod has been running
- **Vectors on Pod** — number of vectors hosted on this pod
- **Search Rate** — searches per second on this pod
- **Indexing Rate** — vectors indexed per second on this pod
...
**KubeDB / Qdrant / Database** — per-collection metrics:
- **Vector Count** — total and indexed vectors per collection
- **Segments** — number of storage segments per collection
- **Disk Usage** — disk space consumed per collection
- **Payload Index Count** — number of indexed payload fields

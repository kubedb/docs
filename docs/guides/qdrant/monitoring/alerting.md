...
* Before proceeding, complete the [Configuration](/docs/guides/qdrant/monitoring/using-prometheus-operator.md#configuration) steps to deploy **kube-prometheus-stack** and **Panopticon** — Panopticon is required for the **Provisioner Group** alerts below (`KubeDBQdrantPhase...`); skip it if you only need the exporter-based **Database Group** alerts.
...
```bash
$ kubectl apply -f https://github.com/kubedb/docs/raw/{{< param "info.version" >}}/docs/examples/qdrant/monitoring/qd-alert-demo.yaml

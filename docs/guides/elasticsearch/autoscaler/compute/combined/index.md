...
This guide will show you how to use `KubeDB` to autoscale compute resources i.e. cpu and memory of a Elasticsearch standalone database.
...
> **Note:** YAML files used in this tutorial are stored in this [directory](/docs/guides/elasticsearch/autoscaler/compute/combined/yaml) of [kubedb/docs](https://github.com/kubedb/docs) repository.
...
In this section, we are going to deploy a Elasticsearch standalone database with version `4.2.3`.  Then, in the next section we will set up autoscaling for this database using `ElasticsearchAutoscaler` CRD. Below is the YAML of the `Elasticsearch` CR that we are going to create,
...
Here, we are going to set up compute (cpu and memory) autoscaling using a ElasticsearchAutoscaler Object.

#### Create ElasticsearchAutoscaler Object

In order to set up compute resource autoscaling for this standalone database, we have to create a `ElasticsearchAutoscaler` CRO with our desired configuration. Below is the YAML of the `ElasticsearchAutoscaler` object that we are going to create,
...
Status:
  Conditions:
    ...
    Message:               Elasticsearch ops request is vertically scaling database
    ...
    Message:               Successfully Vertically Scaled Combined Resources
    Observed Generation:   1
...
Events:
  ...
  ----    ------                      ----   ----                        -------
  Normal  PauseDatabase               3m30s  KubeDB Enterprise Operator  Pausing Elasticsearch demo/mg-standalone
  Normal  PauseDatabase               3m30s  KubeDB Enterprise Operator  Successfully paused Elasticsearch demo/mg-standalone
  ...
  Normal  Starting                    3m30s  KubeDB Enterprise Operator  Updating Resources of StatefulSet: mg-standalone
  Normal  UpdateStatefulSetResources  3m30s  KubeDB Enterprise Operator  Successfully updated StatefulSets Resources
  Normal  UpdateStandaloneResources   3m     KubeDB Enterprise Operator  Successfully Vertically Scaled Combined Resources
  Normal  ResumeDatabase              3m     KubeDB Enterprise Operator  Resuming Elasticsearch demo/mg-standalone
  Normal  ResumeDatabase              3m     KubeDB Enterprise Operator  Successfully resumed Elasticsearch demo/mg-standalone

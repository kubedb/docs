---
title: Application Level Backup & Restore Elasticsearch | KubeStash
...
# Application Level Backup and Restore Elasticsearch database using KubeStash

KubeStash offers application-level backup and restore functionality for `Elasticsearch` databases. It captures both manifest and data backups of any `Elasticsearch` database in a single snapshot. During the restore process, KubeStash first applies the `Elasticsearch` manifest to the cluster and then restores the data into it.

This guide will give you an overview how you can take application-level backup and restore your `Elasticsearch` databases using `Kubestash`.
...
This section will demonstrate how to take application-level backup of a `Elasticsearch` database. Here, we are going to deploy a `Elasticsearch` database using KubeDB. Then, we are going to back up the database at the application level to a `GCS` bucket. Finally, we will restore the entire `Elasticsearch` database.
...
```bash
$ kubectl apply -f https://github.com/kubedb/docs/raw/{{< param "info.version" >}}/docs/guides/elasticsearch/backup/kubestash/application-level/examples/es-quickstart.yaml
...
```bash
$ kubectl apply -f https://github.com/kubedb/docs/raw/{{< param "info.version" >}}/docs/guides/elasticsearch/backup/kubestash/application-level/examples/backupstorage.yaml
...
```bash
$ kubectl apply -f https://github.com/kubedb/docs/raw/{{< param "info.version" >}}/docs/guides/elasticsearch/backup/kubestash/application-level/examples/retentionpolicy.yaml
...
es-dev-restore   s3-elasticsearch-repo                    Succeeded   7s         2m41s
```

The `Succeeded` phase means that the restore process has been completed successfully.


#### Verify Restored Elasticsearch Manifest:

In this section, we will verify whether the desired `Elasticsearch` database manifest has been successfully applied to the cluster.
...
```bash
$ kubectl get es -n dev es-quickstart
NAME            VERSION        STATUS   AGE
...
So, from the above output, we can see the `info` database we had created in the original database `es-quickstart` has been restored successfully.

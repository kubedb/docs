...
The following is an example of a `MSSQLServer` object which creates PostgreSQL cluster of three servers.
...
The following `MSSQLServer` object will create PostgreSQL cluster with *hot standby* servers.
...
To enable high availability, you need to create PostgreSQL with multiple server. Set `spec.replicas` to more than one in MSSQLServer.
...
WAL data is sent immediately rather than waiting for a whole segment to be produced and shipped.
    ...
    KubeDB PostgreSQL support [Streaming Replication](/docs/guides/mssqlserver/clustering/streaming_replication.md)

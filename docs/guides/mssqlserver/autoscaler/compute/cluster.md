...
# Autoscaling the Compute Resource of a MSSQLServer Cluster Database
...
Here, we are going to deploy a `MSSQLServer` Cluster using a supported version by `KubeDB` operator. Then we are going to apply `MSSQLServerAutoscaler` to set up autoscaling.

#### Deploy MSSQLServer Cluster

In this section, we are going to deploy a MSSQLServer Cluster with version `16.1`. Then, in the next section we will set up autoscaling for this database using `MSSQLServerAutoscaler` CRD. Below is the YAML of the `MSSQLServer` CR that we are going to create,
...
```bash
$ kubectl create -f https://github.com/kubedb/docs/raw/{{< param "info.version" >}}/docs/examples/mssqlserver/autoscaler/compute/ha-mssqlserver.yaml
...
ha-mssqlserver        16.1    Ready    14m
```

Let's check the Pod containers resources,

```bash
$ kubectl get pod -n demo ha-mssqlserver-0 -o json | jq '.spec.containers[].resources'
{
  "limits": {
    "cpu": "200m",
    "memory": "512Mi"
...
```bash
$ kubectl get mssqlserver -n demo ha-mssqlserver -o json | jq '.spec.podTemplate.spec.resources'
{
  "limits": {
    "cpu": "200m",
    "memory": "512Mi"
...
- `spec.databaseRef.name` specifies that we are performing compute resource scaling operation on `ha-mssqlserver` database.
...
```bash
$ kubectl apply -f https://github.com/kubedb/docs/raw/{{< param "info.version" >}}/docs/examples/mssqlserver/autoscaler/compute/msas-compute.yaml
...
Status:
  Checkpoints:
    ...
    Ref:
      ...
      Vpa Object Name:    ha-mssqlserver
    ...
    Memory Histogram:
      Reference Timestamp:  2022-09-17T00:00:00Z
    Ref:
      Container Name:     mssqlserver
      Vpa Object Name:    ha-mssqlserver
    Total Samples Count:  19
    Version:              v3
  Conditions:
    Last Transition Time:  2022-09-16T11:27:07Z
    Message:               Successfully created mssqlserverOpsRequest demo/msops-ha-mssqlserver-6xc1kc
...
msops-ha-mssqlserver-6xc1kc   VerticalScaling   Progressing  7s
```
...
$ kubectl get mssqlserveropsrequest -n demo
NAME                              TYPE              STATUS       AGE
msops-vpa-ha-mssqlserver-z43wc8   VerticalScaling   Successful   3m32s
```
...
```bash
$ kubectl describe mssqlserveropsrequest -n demo msops-vpa-ha-mssqlserver-z43wc8
Name:         msops-ha-mssqlserver-6xc1kc
Namespace:    demo
Labels:       <none>
Annotations:  <none>
...
Status:
  Conditions:
    Last Transition Time:  2022-09-16T11:27:07Z
    Message:               Controller has started to Progress the MSSQLServerOpsRequest: demo/msops-ha-mssqlserver-6xc1kc
    Observed Generation:   1
    Reason:                OpsRequestProgressingStarted
    Status:                True
    Type:                  Progressing
    Last Transition Time:  2022-09-16T11:30:42Z
    Message:               Successfully restarted MSSQLServer pods for MSSQLServerOpsRequest: demo/msops-ha-mssqlserver-6xc1kc
    Observed Generation:   1
    Reason:                SuccessfullyRestatedPetSet
    Status:                True
    Type:                  RestartPetSet
    Last Transition Time:  2022-09-16T11:30:47Z
    Message:               Vertical scale successful for MSSQLServerOpsRequest: demo/msops-ha-mssqlserver-6xc1kc
    Observed Generation:   1
    Reason:                SuccessfullyPerformedVerticalScaling
    Status:                True
    Type:                  VerticalScaling
    Last Transition Time:  2022-09-16T11:30:47Z
    Message:               Controller has successfully scaled the MSSQLServer demo/msops-ha-mssqlserver-6xc1kc
    Observed Generation:   1
...
Events:
  ...
  ----    ------      ----   ----                        -------
  Normal  Starting    8m48s  KubeDB Enterprise Operator  Start processing for MSSQLServerOpsRequest: demo/msops-ha-mssqlserver-6xc1kc
  Normal  Starting    8m48s  KubeDB Enterprise Operator  Pausing MSSQLServer databse: demo/ha-mssqlserver
  Normal  Successful  8m48s  KubeDB Enterprise Operator  Successfully paused MSSQLServer database: demo/ha-mssqlserver for MSSQLServerOpsRequest: msops-ha-mssqlserver-6xc1kc
  Normal  Starting    8m43s  KubeDB Enterprise Operator  Restarting Pod: demo/ha-mssqlserver-0
  Normal  Starting    7m33s  KubeDB Enterprise Operator  Restarting Pod: demo/ha-mssqlserver-1
  Normal  Starting    6m23s  KubeDB Enterprise Operator  Restarting Pod: demo/ha-mssqlserver-2
  Normal  Successful  5m13s  KubeDB Enterprise Operator  Successfully restarted MSSQLServer pods for MSSQLServerOpsRequest: demo/msops-ha-mssqlserver-6xc1kc
  Normal  Successful  5m8s   KubeDB Enterprise Operator  Vertical scale successful for MSSQLServerOpsRequest: demo/msops-ha-mssqlserver-6xc1kc
  Normal  Starting    5m8s   KubeDB Enterprise Operator  Resuming MSSQLServer database: demo/ha-mssqlserver
  Normal  Successful  5m8s   KubeDB Enterprise Operator  Successfully resumed MSSQLServer database: demo/ha-mssqlserver
  Normal  Successful  5m8s   KubeDB Enterprise Operator  Controller has Successfully scaled the MSSQLServer database: demo/ha-mssqlserver
```
...
The above output verifies that we have successfully autoscaled the resources of the MSSQLServer cluster database.

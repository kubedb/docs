...
A Kafka Topology cluster is a group of kafka brokers and controller where each broker acts only as a dedicated broker and each controller acts as only as a dedicated controller. The controllers participate in leader election as voters. Topology mode is suitable for production environment where each of the kafka instances runs with their assigned roles and configurations.
...
> Note: YAML files used in this tutorial are stored in [here](https://github.com/kubedb/docs/tree/{{< param "info.version" >}}/docs/guides/kafka/clustering/combined-cluster/yamls) in GitHub repository [kubedb/docs](https://github.com/kubedb/docs).

## Create Kafka Topology Cluster

You should deploy at least three replicas for the Kafka controllers to ensure high availability and fault tolerance. If one of the controllers fails, the remaining controllers can continue to function and maintain the state of the Kafka cluster. The number of replicas for Kafka brokers will depend on your specific use case and workload requirements. Generally, you should deploy multiple replicas for each broker to ensure high availability and fault tolerance.

Here, we are going to create a TLS secured Kafka cluster in Topology mode with 3 controllers and 3 brokers. For this demo, we are going to provision kafka version `3.4.0`. To learn more about Kafka CR, visit [here](/docs/guides/kafka/concepts/kafka/index.md). visit [here](/docs/guides/kafka/concepts/catalog/index.md) to learn more about KafkaVersion CR.

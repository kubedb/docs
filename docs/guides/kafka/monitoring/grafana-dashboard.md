...
$ helm upgrade -i kubedb-grafana-dashboards appscode/kubedb-grafana-dashboards \
    ...
    --set featureGates.Kafka=true \
    --set grafana.name=grafana \
    --set grafana.namespace=monitoring
```

`featureGates.Kafka` already defaults to `true` — set explicitly above for clarity. `grafana.name`/`grafana.namespace` point the chart's `GrafanaDashboard` resources at the `AppBinding` created in step 2 (omitting them falls back to whichever `AppBinding` in your cluster is labeled as the cluster-default Grafana, if any — explicit is safer on a shared cluster).

This creates `KubeDB / Kafka / Summary`, `KubeDB / Kafka / Pod`, and `KubeDB / Kafka / Database` (plus a `ConnectCluster` set, unrelated to plain Kafka) — which `grafana-operator` then pushes into your Grafana instance automatically. No manual JSON download or upload needed.

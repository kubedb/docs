...
> These two steps — deploying `kube-prometheus-stack` and installing Panopticon — are shared prerequisites for all KubeDB database monitoring guides. If you have already completed them in another guide, skip to [Step 1](#step-1-deploy-perconaxtradb-with-monitoring-enabled).
...
## Step 1: Deploy PerconaXtraDB with Monitoring Enabled
...
## Step 5: Import KubeDB PerconaXtraDB Dashboard — Option A: Automatically, via the `kubedb-grafana-dashboards` chart
...
$ helm upgrade -i kubedb-grafana-dashboards appscode/kubedb-grafana-dashboards \
    ...
    --set featureGates.PerconaXtraDB=true \
    --set grafana.name=grafana \
    --set grafana.namespace=monitoring
```

`featureGates.PerconaXtraDB` already defaults to `true` — set explicitly above for clarity. `grafana.name`/`grafana.namespace` point the chart's `GrafanaDashboard` resources at the `AppBinding` created in step 2 (omitting them falls back to whichever `AppBinding` in your cluster is labeled as the cluster-default Grafana, if any — explicit is safer on a shared cluster).

This single command creates every dashboard this chart ships for PerconaXtraDB — `KubeDB / PerconaXtraDB / Summary`, `KubeDB / PerconaXtraDB / Pod`, `KubeDB / PerconaXtraDB / Database`, `KubeDB / PerconaXtraDB / Galera-Cluster` — which `grafana-operator` then pushes into your Grafana instance automatically. No manual JSON download or upload needed.
...
## Step 5: Import KubeDB PerconaXtraDB Dashboard — Option B: Manually, by uploading JSON files

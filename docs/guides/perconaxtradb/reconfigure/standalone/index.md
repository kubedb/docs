---
title: Reconfigure PerconaXtraDB Standalone
menu:
  docs_{{ .version }}:
    identifier: guides-perconaxtradb-reconfigure-standalone
    name: Standalone
...
# Reconfigure PerconaXtraDB Standalone Database

This guide will show you how to use `KubeDB` Enterprise operator to reconfigure a PerconaXtraDB Standalone.
...
Now, we are going to deploy a  `PerconaXtraDB` Standalone using a supported version by `KubeDB` operator. Then we are going to apply `PerconaXtraDBOpsRequest` to reconfigure its configuration.

### Prepare PerconaXtraDB Standalone

Now, we are going to deploy a `PerconaXtraDB` Standalone database with version `10.6.4`.
...
```bash
$ kubectl create -f https://github.com/kubedb/docs/raw/{{< param "info.version" >}}/docs/guides/perconaxtradb/reconfigure/standalone/examples/sample-pxc-config.yaml
...
```bash
$ kubectl apply -f https://github.com/kubedb/docs/raw/{{< param "info.version" >}}/docs/guides/perconaxtradb/reconfigure/standalone/examples/reconfigure-using-secret.yaml
...
root@sample-pxc-0:/# mysql -u${MYSQL_ROOT_USERNAME} -p${MYSQL_ROOT_PASSWORD}
Welcome to the PerconaXtraDB monitor.  Commands end with ; or \g.
Your PerconaXtraDB connection id is 21
Server version: 10.6.4-PerconaXtraDB-1:10.6.4+maria~focal perconaxtradb.org binary distribution
...
```bash
$ kubectl apply -f https://github.com/kubedb/docs/raw/{{< param "info.version" >}}/docs/guides/perconaxtradb/reconfigure/standalone/examples/mdops-reconfigure-apply-config.yaml
...
root@sample-pxc-0:/# mysql -u${MYSQL_ROOT_USERNAME} -p${MYSQL_ROOT_PASSWORD}
Welcome to the PerconaXtraDB monitor.  Commands end with ; or \g.
Your PerconaXtraDB connection id is 24
Server version: 10.6.4-PerconaXtraDB-1:10.6.4+maria~focal perconaxtradb.org binary distribution
...
```bash
$ kubectl apply -f https://github.com/kubedb/docs/raw/{{< param "info.version" >}}/docs/guides/perconaxtradb/reconfigure/standalone/examples/reconfigure-remove.yaml
...
root@sample-pxc-0:/ mysql -u${MYSQL_ROOT_USERNAME} -p${MYSQL_ROOT_PASSWORD}
Welcome to the PerconaXtraDB monitor.  Commands end with ; or \g.
Your PerconaXtraDB connection id is 8
Server version: 10.6.4-PerconaXtraDB-1:10.6.4+maria~focal perconaxtradb.org binary distribution

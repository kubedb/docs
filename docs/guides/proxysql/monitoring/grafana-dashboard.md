---
title: Visualize ProxySQL Metrics with Grafana Dashboard
menu:
  docs_{{ .version }}:
...
# Visualize ProxySQL Metrics with Grafana Dashboard

KubeDB exposes ProxySQL metrics through a sidecar exporter. Once Prometheus scrapes those metrics, you can visualize them in Grafana using a pre-built KubeDB dashboard. This tutorial walks through the full setup: deploying the monitoring stack, enabling monitoring on a ProxySQL instance, and importing the Grafana dashboard.
...
> These two steps — deploying `kube-prometheus-stack` and installing Panopticon — are shared prerequisites for all KubeDB database monitoring guides. If you have already completed them in another guide, skip to [Step 1](#step-1-deploy-proxysql-with-monitoring-enabled).
...
## Step 1: Deploy ProxySQL with Monitoring Enabled
...
proxysql-grafana-demo         ClusterIP   10.96.10.1     <none>        6033/TCP    2m
proxysql-grafana-demo-stats   ClusterIP   10.96.10.2     <none>        6070/TCP    2m
```
...
## Step 5: Import KubeDB ProxySQL Dashboard — Option A: Automatically, via the `kubedb-grafana-dashboards` chart
...
$ helm upgrade -i kubedb-grafana-dashboards appscode/kubedb-grafana-dashboards \
    ...
    --set featureGates.ProxySQL=true \
    --set grafana.name=grafana \
    --set grafana.namespace=monitoring
```

`featureGates.ProxySQL` already defaults to `true` — set explicitly above for clarity. `grafana.name`/`grafana.namespace` point the chart's `GrafanaDashboard` resources at the `AppBinding` created in step 2 (omitting them falls back to whichever `AppBinding` in your cluster is labeled as the cluster-default Grafana, if any — explicit is safer on a shared cluster).

This single command creates every dashboard this chart ships for ProxySQL — `KubeDB / ProxySQL / Summary`, `KubeDB / ProxySQL / Pod`, `KubeDB / ProxySQL / Database` — which `grafana-operator` then pushes into your Grafana instance automatically. No manual JSON download or upload needed.
...
## Step 5: Import KubeDB ProxySQL Dashboard — Option B: Manually, by uploading JSON files

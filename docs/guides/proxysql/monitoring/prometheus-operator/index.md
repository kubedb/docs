...
The sidecar exporter deployed with this ProxySQL instance only reports ProxySQL-native metrics (connections, queries, memory, etc.). The KubeDB ProxySQL Grafana dashboards (see [ProxySQL Grafana Dashboard](/docs/guides/proxysql/monitoring/grafana-dashboard.md)) also chart KubeDB's own view of the resource — database status, phase, version, deletion policy — and those come from a **separate** metric source: **Panopticon**, the Appscode operator that exports `kubedb_com_proxysql_*` metrics for every KubeDB object. Skip this step and the dashboards will still load, but the General Info / phase panels will show "No data".

Install Panopticon with the `prometheus.io/operator` monitoring agent — the same agent this tutorial already uses for ProxySQL itself, so Panopticon's metrics get picked up the same way, via a `ServiceMonitor` the chart creates automatically. No manual Prometheus config editing needed, unlike the [built-in Prometheus](/docs/guides/proxysql/monitoring/builtin-prometheus/) tutorial.
...
> The `monitoring.serviceMonitor.labels.release` value must match whatever label your `Prometheus` CR's `serviceMonitorSelector` expects — `release: prometheus` here, matching the label already used for ProxySQL's own `ServiceMonitor` earlier in this tutorial. If your Prometheus server uses a different `serviceMonitorSelector` label, use that instead.

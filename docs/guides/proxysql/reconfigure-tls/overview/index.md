---
title: Reconfiguring TLS of ProxySQL Database
menu:
  docs_{{ .version }}:
...
# Reconfiguring TLS of ProxySQL Database

This guide will give an overview on how KubeDB Enterprise operator reconfigures TLS configuration i.e. add TLS, remove TLS, update issuer/cluster issuer or Certificates and rotate the certificates of a `ProxySQL` database.
...
4. Then, in order to reconfigure the TLS configuration of the `ProxySQL` database the user creates a `ProxySQLOpsRequest` CR with desired information.
...
8. Then the `KubeDB` Enterprise operator will restart all the Pods of the database so that they restart with the new TLS configuration defined in the `ProxySQLOpsRequest` CR.
...
In the next docs, we are going to show a step by step guide on reconfiguring TLS configuration of a ProxySQL database using `ProxySQLOpsRequest` CRD.